...
function updateOnlineUsers() {
    ...
    $currentTime = time();
    $users = [];
    if(file_exists($usersFile)) $users = unserialize(file_get_contents($usersFile));
...
function handleChatMessage() {
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if(!empty($message) || !empty($imgUrl)) {
            $time = date('H:i');
            $entry = ['time' => $time, 'username' => $username, 'message' => htmlspecialchars($message), 'img_url' => $imgUrl];
            $messages = [];
            if(file_exists($chatFile)) $messages = unserialize(file_get_contents($chatFile));
...
function getChatMessages() {
    global $chatFile;
    return file_exists($chatFile) ? unserialize(file_get_contents($chatFile)) : [];
}
function clearChatMessages() {
    global $chatFile;
    file_put_contents($chatFile, serialize([]));
    return true;
}

if($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['action']) && $_POST['action'] === 'send_message') { handleChatMessage(); header('Content-Type: application/json'); echo json_encode(['status' => 'success']); exit; }
    ...
    elseif(isset($_POST['action']) && $_POST['action'] === 'get_online_count') { $count = getOnlineUsersCount(); header('Content-Type: application/json'); echo json_encode(['count' => $count]); exit; }
    elseif(isset($_POST['action']) && $_POST['action'] === 'clear_chat') { clearChatMessages(); header('Content-Type: application/json'); echo json_encode(['status' => 'success']); exit; }
}
...
<head>
    ...
    <style type="text/tailwindcss">
        @layer utilities {
            ...
            .left-no-scroll::-webkit-scrollbar { display: none !important; }
...
<body class="bg-gray-100 font-sans text-gray-800">
    ...
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- 左侧：播放器+音量按钮固定显示 -->
            <div class="lg:col-span-2 left-no-scroll">
                <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                    ...
                    <div class="p-4 md:p-6">
                        ...
                        <div class="mt-4 md:mt-6">
                            ...
                            <div class="grid grid-cols-5 gap-2 md:gap-4 items-center">
                                <button id="volume-down" class="bg-gray-100 hover:bg-gray-200 p-2 md:p-3 rounded-lg transition-all flex items-center justify-center">
                                ...
                                <button id="volume-up" class="bg-gray-100 hover:bg-gray-200 p-2 md:p-3 rounded-lg transition-all flex items-center justify-center">
                                    <i class="fa fa-volume-up text-gray-700"></i>
                                </button>
                            </div>
                            <div class="mt-4 grid grid-cols-2 gap-2 md:gap-4">
                                <button id="volume-mute" class="bg-gray-100 hover:bg-gray-200 p-2 md:p-3 rounded-lg transition-all flex items-center justify-center">
                                    <i class="fa fa-volume-off text-gray-700"></i>
                                    <span class="ml-2">静音</span>
                                </button>
                                <button id="fullscreen" class="bg-gray-100 hover:bg-gray-200 p-2 md:p-3 rounded-lg transition-all flex items-center justify-center">
                ...
                <div class="mt-6 bg-white rounded-xl shadow-lg p-4 md:p-6">
                    ...
                    <div class="mt-4 grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div class="bg-gray-50 p-3 rounded-lg text-center"><i class="fa fa-clock-o text-primary text-xl mb-2"></i><p class="text-sm">全天候播放</p></div>
                        <div class="bg-gray-50 p-3 rounded-lg text-center"><i class="fa fa-music text-primary text-xl mb-2"></i><p class="text-sm">精选曲目</p></div>
                        <div class="bg-gray-50 p-3 rounded-lg text-center"><i class="fa fa-wifi text-primary text-xl mb-2"></i><p class="text-sm">高清音质</p></div>
                        <div class="bg-gray-50 p-3 rounded-lg text-center"><i class="fa fa-comments text-primary text-xl mb-2"></i><p class="text-sm">互动交流</p></div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-1 h-full overflow-hidden">
                <div class="bg-white rounded-xl shadow-lg h-full flex flex-col">
                    <div class="p-3 border-b border-gray-200 flex justify-between items-center shrink-0">
                        <h3 class="text-base font-semibold flex items-center"><i class="fa fa-comments text-accent mr-2"></i> 听众聊天室</h3>
                        <button id="clear-chat" class="bg-red-500 hover:bg-red-600 text-white px-2 py-1 rounded-lg text-xs transition-all"><i class="fa fa-trash mr-1"></i>清屏</button>
                    </div>
                    ...
                    <div class="p-3 border-t border-gray-200 shrink-0">
                        <div class="mb-2">
                            <input type="text" id="username" placeholder="请输入您的昵称" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-primary">
                        </div>
                        <div class="flex items-center gap-1">
                            <input type="file" id="image-upload" accept="image/*" class="hidden">
                            <button id="send-image" class="bg-indigo-500 hover:bg-indigo-600 text-white px-2 py-1.5 rounded-lg text-xs transition-all"><i class="fa fa-image"></i></button>
                            <input type="text" id="message-input" placeholder="输入消息..." class="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-primary">
                            <button id="send-message" class="bg-primary hover:bg-primary-600 text-white px-3 py-2 rounded-lg text-sm transition-all"><i class="fa fa-paper-plane"></i></button>
                        </div>
    ...
    <script>
        ...
        function updateVolumeDisplay() {
            ...
            if (audio.muted || percentage === 0) {
                volumeMuteBtn.innerHTML = '<i class="fa fa-volume-off text-gray-700"></i><span class="ml-2">取消静音</span>';
            } else {
        ...
        volumeSlider.addEventListener('input', () => {
            ...
            if (!audio.muted) lastVolume = volume;
            updateVolumeDisplay();
        });

        fullscreenBtn.addEventListener('click', () => {
            const playerContainer = document.querySelector('.player-gradient').closest('.bg-white');
                ...
                reader.onload = function(e) {
                    ...
                    img.onload = function() {
                        ...
                        if(width > maxSide || height > maxSide) {
                            if(width > height) { width = maxSide; height = (img.height * maxSide) / img.width; }
                            else { height = maxSide; width = (img.width * maxSide) / img.height; }
                        }
                        canvas.width = width;
                        ...
                        function getCompressed() {
                            ...
                            if(sizeKB > maxSizeKB && quality > 0.5) { quality -= 0.05; getCompressed(); }
        ...
        imageUpload.addEventListener('change', async (e) => {
            ...
            xhr.upload.onprogress = function(e) { if (e.lengthComputable) uploadProgressBar.style.width = (e.loaded / e.total) * 100 + '%'; };
            xhr.onload = function() { uploadProgress.style.display = 'none'; imageUpload.value = ''; };
        ...
        function addLocalImageMsg(name, imgSrc) {
            ...
            el.innerHTML = `
                <div class="flex items-start">
                    <div class="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-primary text-xs"><i class="fa fa-user"></i></div>
                    <div class="ml-2 flex-1">
                        <div class="flex items-center gap-1"><span class="text-xs font-medium">${name}</span><span class="text-[10px] text-gray-400">${nowTime}</span></div>
                        <img src="${imgSrc}" class="mt-1 max-h-32 rounded-md cursor-pointer chat-img" data-src="${imgSrc}">
                    </div>
                </div>
            `;
            chatMessages.appendChild(el);
            el.querySelector('.chat-img').onclick = function(){ bigImg.src = this.dataset.src; imgMask.style.display = 'flex'; };
        ...
        function sendMessage() {
            ...
            }).then(res=>res.json()).then(()=>{ messageInput.value=''; loadMessages(); });
        }
        ...
        function loadMessages() {
            fetch('', {
                ...
                if(messages.length===0){ chatMessages.innerHTML = '<div class="text-center text-gray-400 text-sm py-4">暂无消息</div>'; return; }
                messages.forEach(m=>{
                    const el = document.createElement('div');
                    el.className = 'mb-2 pb-2 border-b border-gray-100';
                    let html = `
                    <div class="flex items-start">
                        <div class="w-6 h-6 rounded-full bg-primary/10 flex items-center justify-center text-primary text-xs"><i class="fa fa-user"></i></div>
                        <div class="ml-2 flex-1">
                            <div class="flex items-center gap-1"><span class="text-xs font-medium">${m.username}</span><span class="text-[10px] text-gray-400">${m.time}</span></div>
                    `;
                ...
                document.querySelectorAll('.chat-img').forEach(img=>{ img.onclick = function(){ bigImg.src = this.dataset.src; imgMask.style.display = 'flex'; } });
                chatMessages.scrollTop = chatMessages.scrollHeight;
            });
        }
